...
# Global variables for undo/redo functionality
undo_stack = []
redo_stack = []
selected_directory = None
scan_mode = "Files"  # Default scan mode
sort_by = "Name"  # Default sort criteria

def list_items(directory, filter_text="", sort_by="Name"):
    """List all files or folders in the given directory with optional filtering and sorting."""
    try:
        if scan_mode == "Files":
...
def rename_items(directory, old_names, new_names):
    """Rename files or folders in the directory from old_names to new_names."""
    try:
        for old_name, new_name in zip(old_names, new_names):
            ...
            if os.path.exists(old_path):
                ...
                undo_stack.append((new_name, old_name))  # Push to undo stack
            else:
...
def batch_rename_items():
    """Batch rename items based on pasted new names."""
    ...
    try:
        # Save the current state before batch rename
        redo_stack.clear()  # Clear redo stack since we are performing a new action
...
def undo():
    ...
    try:
        while undo_stack:
            ...
            if os.path.exists(old_path):
                os.rename(old_path, new_path)
                redo_stack.append((old_name, new_name))  # Push to redo stack
            else:
...
def redo():
    """Redo the last undone renaming operation."""
    if not redo_stack:
...
def start_scan_thread(directory, filter_text=""):
    """Start a thread for scanning the directory to prevent freezing."""
...
def update_item_list(directory, filter_text=""):
    """Update the file or folder list in the UI with optional filtering."""
...
def show_loading_indicator(thread):
    """Show a loading indicator while the thread is running."""
...
def copy_to_clipboard():
    """Copy the list of items to the clipboard."""
...
def filter_items():
    """Filter the file or folder list based on the filter text."""
    filter_text = filter_entry.get().strip()
    start_scan_thread(selected_directory, filter_text)

def set_scan_mode():
    """Set the scan mode based on the selected radio button."""
    global scan_mode
...
def set_sort_by(event):
    """Set the sort criteria based on the selected option."""
    global sort_by
